...
# - TOPLEVEL에 connection을 import하지 않음 (스캐너/초기화 이슈 회피)
# ─────────────────────────────────────────────────────────────
...
# ─────────────────────────────────────────────────────────────
_LEGALCFG_CACHE_TTL = _safe_int(os.environ.get("LEGALCFG_CACHE_TTL", "30"), default=30)  # seconds
...
def build_legal_context() -> dict:
    ...
    - settings 기본값 우선 → LegalConfig(DB)가 있으면 override
    - 소문자 키: 기본
    - 대문자 키: 구 템플릿 호환(SERVICE_NAME 등)
    """
    # 1) settings 기본값(운영 일관성)
    service_name_s = getattr(settings, "SERVICE_NAME", None) or getattr(settings, "SERVICE_TITLE", None) or "김동건의 포트폴리오"
    operator_name_s = getattr(settings, "OPERATOR_NAME", None) or "김동건"
    contact_email_s = getattr(settings, "CONTACT_EMAIL", None) or "kdg283@gmail.com"
    contact_phone_s = getattr(settings, "CONTACT_PHONE", None) or ""

    # LEGAL_EFFECTIVE_DATE는 settings에서 이미 string으로 갖고 있는 케이스가 많아서 우선 사용
    effective_date_s = getattr(settings, "LEGAL_EFFECTIVE_DATE", None) or getattr(settings, "EFFECTIVE_DATE", None) or "2025-11-12"

    # 2) DB LegalConfig (있으면 override)
    try:
        cfg = get_active_legal_config_cached()
    except Exception:
        cfg = None

    # 이름/연락처/서비스명: DB 값이 있으면 우선
    service_name = getattr(cfg, "service_name", None) or service_name_s
    operator_name = getattr(cfg, "operator_name", None) or operator_name_s
    contact_email = getattr(cfg, "contact_email", None) or contact_email_s
    contact_phone = getattr(cfg, "contact_phone", None) or contact_phone_s

    # 시행일: DB effective_date가 있으면 우선, 없으면 settings
    effective_date_obj = getattr(cfg, "effective_date", None)
    if effective_date_obj is not None and getattr(effective_date_obj, "isoformat", None):
        effective_date = effective_date_obj.isoformat()
    else:
        effective_date = str(effective_date_s)

    # HTML 문서: DB에 있으면 sanitize 후 사용
...
def validate_required_consents(request) -> Tuple[bool, Optional[str]]:
    ...
    # 2) LegalConfig 없으면 게이트 없음
    cfg = get_active_legal_config_cached()
    if cfg is None:
        return True, None

    # 3) 게이트 on인지 확인(필드 존재 유무 안전)
    gate_on = False
