...
def api_media_pending_reject(request: HttpRequest) -> JsonResponse:
    ...
    try:
        ...
        log.exception("failed to write rejected meta for pending_id=%s key=%s", pending_id, rejected_key)

    try:
        if default_storage.exists(meta_key):
            default_storage.delete(meta_key)
    except Exception:
        log.exception("failed to delete pending meta for pending_id=%s key=%s", pending_id, meta_key)
